...
def gen(data_file, image_path, batchsize=128, maxlabellength=10, imagesize=(32, 280)):
    ...
    while True:
        ...
        for i, j in zip(range(batchsize), shufimagefile):
            ...
            img = imutils.resize(img, height=32)
            h, w = img.shape[:2]
            img = cv2.copyMakeBorder(img, 0, 0, 0, 280 - w, cv2.BORDER_CONSTANT, value=0)
            # if i % 4 == 0:
...
if __name__ == '__main__':
    ...
    modelPath = '../network/models/weights-densenet.h5'
    # modelPath = './models/sheet_vin/weights-densenet-107-0.75.h5'
    train_txt = '../datasets/sheet_vin/train.txt'
    val_txt = '../datasets/sheet_vin/val.txt'
    image_path = '../datasets/sheet_vin/images/'
    EPOCH = 200

    try:
        INITIAL_EPOCH = int(modelPath.split('-')[-2])
        print('Restarting at EPOCH {}'.format(INITIAL_EPOCH))
    except:
        INITIAL_EPOCH = 0
    TRAIN_IMAGE_COUNT = len(open(train_txt).readlines())
    VAL_IMAGE_COUNT = len(open(val_txt).readlines())

    lr_schedule = lambda epoch: 0.5 * 0.95 ** epoch  # 1-10, batchsize 128
    ...
    checkpoint = ModelCheckpoint(filepath='./models/sheet_vin/weights-densenet-{epoch:02d}-{val_acc:.2f}.h5', monitor='val_acc',
    ...
    tensorboard = TensorBoard(log_dir='./models/sheet_vin/logs', write_graph=True)
